...
class FixInt(fixer_base.BaseFix):
    ...
    PATTERN = u"'int' | NUMBER"

    static_long = Name(u"long")

    def base(self, literal):
        u"""Returns the base of a valid py3k numeric literal."""
    ...
    def unmatch(self, node):
        u"""Don't match complex numbers, floats, or longs"""
        val = node.value
    ...
    def transform(self, node, results):
        ...
        if node.type == token.NUMBER and self.base(val) == 10:
            assert not val[-1] in u"lL", u"Invalid py3k literal: " + unicode(val)
            val += u"L"
            return Number(val, prefix=node.prefix)
        elif is_probably_builtin(node):
            assert node.type == token.NAME, u"Sanity check failed: " + unicode(val)
