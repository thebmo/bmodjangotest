...
class FixBytes(fixer_base.BaseFix):
    ...
    def transform(self, node, results):
        ...
        if arglist is not None:
            ...
            source, encoding, errors = (parsed[v] for v in (u"source", u"encoding", u"errors"))
            encoding.prefix = u""
            str_call = Call(Name(u"str"), ([source.clone()]))
            if errors is None:
                node.replace(Call(Name(unicode(str_call) + u".encode"), (encoding.clone(),)))
            else:
                errors.prefix = u" "
                node.replace(Call(Name(unicode(str_call) + u".encode"), (encoding.clone(), Comma(), errors.clone())))
