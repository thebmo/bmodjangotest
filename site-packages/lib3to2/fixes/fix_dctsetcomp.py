...
class FixDctsetcomp(fixer_base.BaseFix):
    ...
    PATTERN = u"""atom< '{' dictsetmaker< 
                  n1=any [col=':' n2=any]
                    comp_for=comp_for< 'for' any 'in' any [comp_if<'if' any>] >
                  > '}' >"""

    def transform(self, node, results):
        comp_for = results.get(u"comp_for").clone()
        is_dict = bool(results.get(u"col")) # is it a dict?
        n1 = results.get(u"n1").clone()
        if is_dict:
            n2 = results.get(u"n2").clone()
            n2.prefix = u" "
