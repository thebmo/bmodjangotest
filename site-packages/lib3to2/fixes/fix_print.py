       ...
       not (sep.type == token.STRING and sep.value in (u"' '", u'" "')):
        ...
        for arg in pos:
            ...
            if sys.version_info >= (2, 6):
                warnings.warn(u"Calling unicode() on what may be a bytes object")
            temp.append(Comma())
        del temp[-1]
        sep = sep.clone()
        sep.prefix = u" "
...
class FixPrint(fixer_base.BaseFix):
    ...
    def transform(self, node, results):
        ...
        if pos is None or opts is None:
            self.cannot_convert(node, u"-fprint does not support argument unpacking.  fix using -xprint and then again with  -fprintfunction.")
            return
        ...
        try:
            with warnings.catch_warnings(record=True) as w:
                ...
                if len(w) > 0:
                    self.warning(node, u"coercing to unicode even though this may be a bytes object")
        except AttributeError:
            # Python 2.5 doesn't have warnings.catch_warnings, so we're in Python 2.5 code here...
            new_node = replace_print(pos, dict([(str(k), opts[k]) for k in opts]), old_node=node)
